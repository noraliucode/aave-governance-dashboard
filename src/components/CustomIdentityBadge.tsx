...
function CustomIdentityBadge({
  entity,
  connectedAccount,
  label,
  shorten,
}: CustomIdentityBadgeProps) {
  const theme = useTheme();

  const [storedLabels, setAllLabels] = useState<
  ...
  useEffect(() => {
    const customLabelOrUndefined = storedLabels.find(
      (entry) => entry.address === entity
    )?.label;
  ...
  const onClickSave = () => {
    ...
    const newInfos = storedLabels
      .filter((labels) => labels.address !== entity)
      .concat(entry);
